...
LDM::LDM(string globalConfig, string loggingConf, string statisticConf) {
	...
	/* 
		The denservice is the central service
		It receives the DENM ECITS messages and forwards them accordingly
		To other modules, like the LDM module, so LDM has this receiver
		Corresponding to DENM, but ECITS channels
	 */
	mReceiverFromEcits = new CommunicationReceiver(moduleName, "9999", "ECITS", config.mExpNo, loggingConf, statisticConf);
...
LDM::~LDM() {
	...
	delete mThreadReceiveFromEcits;
...
void LDM::init() {
	...
	// Thread for Receiver ECITS
	mThreadReceiveFromEcits = new boost::thread(&LDM::receiveFromEcits, this);
...
/* 
	SQL Functions to create TABLES in Database
	Created Feature table for ECITS
 */
...
void LDM::createTables() {
	...
	/* 
		If using confidence levels, i.e.: epx/epy, would create those columns
	 */
	sqlCommand = (char*) "CREATE TABLE IF NOT EXISTS FEATURES(" \
				"id INTEGER PRIMARY KEY, latitude DOUBLE, longitude DOUBLE, altitude DOUBLE, detection_time INTEGER, consensus_reached INTEGER, h_proposed_id INTEGER);";
	if (sqlite3_exec(mDb, sqlCommand, NULL, 0, &errmsg)) {
		string error(errmsg);
		mLogger->logError("SQL error: " + error);
		sqlite3_free(errmsg);
...
/* 
	SELECT ON FEATURES Table
	--> Possible to use a condition
 */
Json::Value LDM::featureSelect(string condition) {

	Json::Value result;
	Json::Value row;

	sqlite3_stmt *stmt;
	string sqlCommand = "SELECT * from Features " + condition;
	char* errmsg = 0;
	mFeaturesMutex.lock();
	if (sqlite3_prepare_v2(mDb, sqlCommand.c_str(), -1, &stmt, NULL) == SQLITE_OK) {
		...
		while (sqlite3_step(stmt) == SQLITE_ROW) {		
			...
			row["id"] = sqlite3_column_double(stmt,0);
			...
			row["detection_time"] = sqlite3_column_double(stmt, 4);
			row["consensus_reached"] = sqlite3_column_double(stmt, 5);
			row["h_proposed_id"] = sqlite3_column_double(stmt, 6);
			result.append(row);
		}
...
/* 
	...
	- receives serialized feature
 */
void LDM::insertDenmEcits(std::string serializedFeature) {

	Json::Value feature;
	Json::Reader reader;
	reader.parse(serializedFeature,feature);

	stringstream sSql;
	sSql  << setprecision(15);

	const Json::Value id = feature["id"];
	const Json::Value latitude = feature["latitude"];
	const Json::Value longitude = feature["longitude"];
	const Json::Value altitude = feature["altitude"];
	const Json::Value detectionTime = feature["detection_time"];
	const Json::Value consensusReached = feature["consensus_reached"];
	const Json::Value highestProposedID = feature["h_proposed_id"];
	
	sSql << "INSERT OR REPLACE INTO FEATURES (id, latitude, longitude, altitude, detection_time, consensus_reached, h_proposed_id) VALUES ("\
			<< id << ", " << latitude << ", " << longitude << ", " << altitude << ", " << detectionTime << ", " << consensusReached << ", " << highestProposedID << "); ";

	mFeaturesMutex.lock();
	insert(sSql.str());
	mFeaturesMutex.unlock();

	mLogger->logInfo("Inserted/Updated feature with ID:" + feature["id"].asString() + "\n");

}

/* 
	DELETES from Features table entry with specified ID
	- receives feature ID to DELETE
 */
void LDM::deleteDenmEcits(string id){

	stringstream sSql;
	sSql << "DELETE FROM FEATURES WHERE id = " << id;

	mFeaturesMutex.lock();
	insert(sSql.str());
	mFeaturesMutex.unlock();

	mLogger->logInfo("REMOVED Feature if it existed\n ID");
...
void LDM::receiveRequest() {
	...
	while(1) {
		...
		else if (envelope.compare("ECITS") == 0) {
			Json::StreamWriterBuilder wbuilder;
			Json::Value features = featureSelect(request);
			reply = Json::writeString(wbuilder,features);
			mLogger->logInfo(reply);
		}
...
void LDM::receiveFromDen() {
	...
	while (1) {
		...
		mLogger->logInfo(denm.DebugString());

		//Assume: Received denm is newer than all others received before
...
void LDM::receiveFromEcits() {
	...
	Json::Value existentFeature;

	while (1) {
		pair<string, string> received = mReceiverFromEcits->receive();
		envelope = received.first;
		message = received.second;

		reader.parse(message,jsonMessage);

		if(jsonMessage.isMember("phase")){
			mLogger->logInfo("\nPhase 1\n");

			existentFeature = featureSelect("WHERE id = " + jsonMessage["id"].asString());
			...
			if(!existentFeature){
				...
				Json::StreamWriterBuilder wbuilder;
				string same_feat = Json::writeString(wbuilder,existentFeature);

				sendto(sockid, same_feat.c_str(), same_feat.length(),0, (struct sockaddr *) &foreignAddr, sizeof(foreignAddr));
			}
			continue;
		}
		
		if(jsonMessage.size()<2){
			mLogger->logInfo("\nPHASE 3 --> Did not insert any feature -- Empty majority\n");
			if(!jsonMessage["id"].isNull()){
				// mLogger->logInfo("\nID: \n" + jsonMessage["id"].asString());
				deleteDenmEcits(jsonMessage["id"].asString().c_str());
			}
			continue;
		} else {
			mLogger->logInfo("\nPHASE 3 --> Command to insert a Feature -- Non-Empty from by Majority\n");
			insertDenmEcits(message);
		}
	}
}
