...
public:
	...
	/** 
	 * 	Queries Features from the database
	 */
	Json::Value featureSelect(std::string condition);
	...
	/** 
	 * 	INSERTS Features IN Features Table
	 */
	void insertDenmEcits(std::string serializedFeature);
	
	void deleteDenmEcits(string id);
...
private:
	...
	// ECITS
	CommunicationReceiver* mReceiverFromEcits;
	...
	// ECITS
	boost::thread* mThreadReceiveFromEcits;
	...
	// ECITS feature DB Mutex
	std::mutex mFeaturesMutex;
