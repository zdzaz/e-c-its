...
DenService::DenService(string globalConfig, string loggingConf, string statisticConf) {
	...
	mReceiverFromEcits = new CommunicationReceiver(module, "2121", "ECITS", mGlobalConfig.mExpNo, loggingConf, statisticConf,2);
...
void DenService::init() {
	...
	mThreadEcitsDataReceive = new boost::thread(&DenService::receiveEcits, this);
...
void DenService::receive() {
	...
	while(1) {
		...
		mLogger->logInfo("Forward DENM " + to_string(denm->header.stationID) + " to LDM");
		mSenderToLdm->send(envelope, serializedProtoDenm);
	}
}

/* 
	Receive messages from ECITS/Consensus
 */
void DenService::receiveEcits() {
	...
	while(1) {
		...
		mLogger->logInfo("Forwarding DENM " + envelope + " to LDM. Message:" + message);
		// std::this_thread::sleep_for(std::chrono::seconds(1));
		mSenderToLdm->send(envelope, message);
	}
}
